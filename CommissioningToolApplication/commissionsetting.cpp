    ...
    , ui(new Ui::CommissionSetting)
{
    ...
    // this->sizeHint();
    //this->setFixedSize(QLayout::SetFixedSize);
    QString Title = QString("Commissioning Tool V %1.%2.%3").arg(VERSION_MAJ).arg(VERSION_MIN).arg(VERSION_REV);
...
void CommissionSetting::onComboTextChanged(QString port)
{
    ...
    if(ui->comboBox_databases->currentIndex()==-1)
    {
      //  qDebug() <<"index out of range";
        ...
        ui->comboBox_databases->setMinimumWidth(source.length()*14);
...
void CommissionSetting::ReloadLastSelectedIPs()
{
    ...
    ui->comboBox_ConnectedIPs->setMinimumWidth(value.length()*14);
...
//    QString dataPort = ui->lineEdit_DataPort->text();
    ...
    {
        QMessageBox msgBox;
        msgBox.setText("Enter the Data Port Number");
        msgBox.exec();
        return;
    }
...
void CommissionSetting::on_m_PshBtn_PLCAlarmPortSave_clicked()
{
    ...
    {
        QMessageBox msgBox;
        msgBox.setText("Enter the Alarm Port Number");
        msgBox.exec();
        return;
    }
    QJsonObject jsonKeys;
